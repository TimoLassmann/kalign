...
int aln_runner(struct aln_mem* m,struct aln_param* ap, int* path)
{
        ...
        ap->input_states[0] = m->f[0].a;
        ap->input_states[1] = m->f[0].ga;
        ap->input_states[2] = m->f[0].gb;
        ap->input_states[3] = m->b[0].a;
        ap->input_states[4] = m->b[0].ga;
        ap->input_states[5] = m->b[0].gb;

        mid = ((m->enda - m->starta) / 2)+ m->starta;

        ap->old_cor[0] = m->starta;
        ap->old_cor[1] = m->enda;
        ap->old_cor[2] = m->startb;
        ap->old_cor[3] = m->endb;
        ap->old_cor[4] = mid;
        ...
        if(ap->seq1){
                ...
                aln_seqseq_meetup(m,ap,&meet,&transition,&score);
        }else if(ap->prof2){
                aln_profileprofile_backward(m,ap);
                aln_profileprofile_meetup(m,ap,&meet,&transition,&score);
        }else{
                aln_seqprofile_backward(m,ap);
                aln_seqprofile_meetup(m,ap,&meet,&transition,&score);
        }

        if(ap->mode == ALN_MODE_SCORE_ONLY){
                ap->score = score;
        }else{
                aln_continue(m, ap, path, meet, transition);
        }
        return OK;
}

int aln_continue(struct aln_mem* m,struct aln_param* ap,int* path,int meet,int transition)
{
        ...
        case 1: //a -> a = 1
                ...
                path[ap->old_cor[4]] = meet;
                path[ap->old_cor[4]+1] = meet+1;

                //		fprintf(stderr,"Aligning:%d-%d\n",old_cor[4],c);
                //		fprintf(stderr,"Aligning:%d-%d\n",old_cor[4]+1,c+1);
                //foward:
                m->f[0].a = ap->input_states[0];
                m->f[0].ga = ap->input_states[1];
                m->f[0].gb = ap->input_states[2];
                m->b[0].a = 0.0F;
                m->b[0].ga = -FLT_MAX;
                m->b[0].gb = -FLT_MAX;
                //		fprintf(stderr,"Using this for start:%d	%d	%d\n",m->f[0].a,m->f[0].ga,m->f[0].gb);

                m->starta = ap->old_cor[0];
                m->enda = ap->old_cor[4]-1;

                m->startb = ap->old_cor[2];
                m->endb = meet-1;
                //fprintf(stderr,"Following first: %d  what:%d-%d	%d-%d\n",c-1,m->starta,m->enda,m->startb,m->endb);
                aln_runner(m,ap,path);
                //backward:
                m->starta = ap->old_cor[4]+1;
                m->enda = ap->old_cor[1];
                m->startb = meet+1;
                m->endb = ap->old_cor[3];
                m->f[0].a = 0.0F;
                m->f[0].ga = -FLT_MAX;
                m->f[0].gb = -FLT_MAX;
                m->b[0].a = ap->input_states[3];
                m->b[0].ga = ap->input_states[4];
                m->b[0].gb = ap->input_states[5];
