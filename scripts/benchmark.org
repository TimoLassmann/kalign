...
** Scripts to run programs 
   ...
   #+BEGIN_SRC sh :exports both :results none :noweb yes :tangle test.sh :shebang #!/usr/bin/env bash
...
EOT
   ...
   kalign -i %s -f msf -o %s/%s_%s_kalign.msf
   muscle3.8.31_i86linux32 -msf -in %s -out  %s/%s_%s_muscle.msf
   clustalo-1.2.4-Ubuntu-x86_64 --outfmt=msf  --in %s  --out %s/%s_%s_clustalo.msf


   kalign %s -f msf -o %s/%s_%s_%s_kalign.msf
   muscle3.8.31_i86linux32 -msf -in %s -out  %s/%s_%s_%s_muscle.msf
   clustalo-1.2.4-Ubuntu-x86_64 --outfmt=msf  --in %s --force --out %s/%s_%s_%s_clustalo.msf

   kalign -i %s -o %s/%s_kalign.afa
   muscle3.8.31_i86linux32 -in %s -out  %s/%s_muscle.afa
   clustalo-1.2.4-Ubuntu-x86_64 --threads 8 --outfmt=a2m  --in %s  --out %s/%s_clustalo.afa


   //reformatting:

   kalign %s --rename -reformat -f msf -o %s.msf
