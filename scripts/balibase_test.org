...
* Paper figure 
  ...
  #+BEGIN_SRC R :session one 
    assess_diff_alignment_scores <-function(mat, groupname,tests){
      ...
      t = t.test(f$clustalo,f$kalign, paired = TRUE, alternative = "two.sided")
      if(t$p.value * tests <= 0.05){
        message("sig diff: clustalo, kalign ", groupname," ", t$p.value);
        message("Clustalo ", mean(f$clustalo));
        message("Kalign ", mean(f$kalign));
        message("Muscle ", mean(f$muscle));
      }

      t = t.test(f$clustalo,f$muscle, paired = TRUE, alternative = "two.sided")
      if(t$p.value * tests  <= 0.05){
        message("sig diff: clustalo, muscle ", groupname," ", t$p.value);
        message("Clustalo ", mean(f$clustalo));
        message("Kalign ", mean(f$kalign));
        message("Muscle ", mean(f$muscle));
      }

      t =t.test(f$kalign,f$muscle, paired = TRUE, alternative = "two.sided")
      if(t$p.value *tests <= 0.05){
