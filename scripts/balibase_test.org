...
** Step 3: plot scores 
   ...
   #+BEGIN_SRC R :session  one :results none :export none 
          ...
          mat <- read.table("scores2.csv") 
          colnames(mat) <- c("Type","Name","SP","TC")

     mat$Type <- mat$Name
          mat$Type <- sub(".*kalign[_]cmsa.*", "cmsa", mat$Type, ignore.case = TRUE)
     mat$Type <- sub(".*kalign.msf", "kalign", mat$Type, ignore.case = TRUE)
     mat$Type <- sub(".*clustalo.*", "clustalo", mat$Type, ignore.case = TRUE)
...
f <- ggplot(mat, aes(Group, SP))
f <- f + geom_boxplot(aes(colour = Type))
