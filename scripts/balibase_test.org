...
** Step 2: run tests
   ...
   #+BEGIN_SRC sh :session onesh :results raw 
     find ~/data/bb3_release -name "*.xml" | awk -v outdir="$KALIGNOUTDIR" '{n=split ($1,a,/[\/,.]/); printf "kalign -alnp /home/user/code/kalign/dev/seed_param.txt %s -o %s/%s_%skalign.msf\n", $1,outdir,a[n-2],a[n-1] }' > run_kalign.sh
...
mat <- rbind(mat,readBaliscores("scores_kalign_alnp.csv","kalign alnp"));
